  ...
  ngOnInit(): void {
    ...
    if (params.get('ownerPublicKey')) {
      ...
      if (
        !this.accountService
          .getPublicKeys()
          .includes(this.ownerPublicKeyBase58Check)
      ) {
    ...
    if (params.get('derivedPublicKey')) {
      this.derivedPublicKeyBase58Check = params.get(
        'derivedPublicKey'
      ) as string;
    } else {
    ...
    if (params.get('JWT')) {
      ...
      try {
        const derivedPublicKey = this.cryptoService
          .publicKeyToECBuffer(this.derivedPublicKeyBase58Check)
          .toString('hex');
    ...
    if (params.get('messagePublicKeys')) {
      try {
        this.messagePublicKeys = (
          params.get('messagePublicKeys') as string
        ).split(',');
        for (const messageKey of this.messagePublicKeys) {
          const publicKeyBytes =
            this.cryptoService.publicKeyToECBuffer(messageKey);
        }
      } catch (e) {
        console.error(e);
    ...
    this.backendApi
      .GetUserDerivedKeys(this.ownerPublicKeyBase58Check)
      .subscribe((keys) => {
        if (this.derivedPublicKeyBase58Check in keys) {
          this.backendApi.GetAppState().subscribe((state) => {
            ...
            if (!key) {
              this.errorMsg =
                'This should never happen but if it does, DH was right.';
              return;
            }
            if (blockHeight < key.expirationBlock && key.isValid) {
              const sharedSecrets: string[] = [];
              for (const messageKey of this.messagePublicKeys) {
                const sharedSecret = this.accountService.getPrivateSharedSecret(
